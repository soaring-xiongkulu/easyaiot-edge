...
说明:
    ...
    - 脚本会先删除所有表，然后导入 .scripts/postgresql/iot-edge-ai10.sql 文件到 iot-edge-ai20 数据库
...
def interactive_confirm(tables):
    ...
    print("\n⚠️  此操作会永久删除所有数据，无法恢复！")
    print("删除后将自动导入 .scripts/postgresql/iot-edge-ai10.sql 文件到 iot-edge-ai20 数据库")
    print("\n请确认是否继续删除和导入操作？")
...
# 导入SQL文件
def import_sql_file(db_info, sql_file_path, target_database='iot-edge-ai20'):
    """使用psql命令导入SQL文件"""
...
def main():
    ...
    # 使用iot-edge-ai20作为目标数据库
    db_info['database'] = 'iot-edge-ai20'
    ...
    print(f"   目标数据库: iot-edge-ai20")
    print()
    
    # 创建数据库引擎（用于删除表）
    try:
        # 修改数据库URL以连接到iot-edge-ai20数据库
        db_url_for_drop = re.sub(r'/([^/]+)(\?|$)', f'/iot-edge-ai20\\2', database_url_for_sqlalchemy)
    ...
    sql_file_path = os.path.join(project_root, '.scripts', 'postgresql', 'iot-edge-ai10.sql')
    
    import_success = import_sql_file(db_info, sql_file_path, target_database='iot-edge-ai20')
