...
#                    - build: 不指定则构建全部 4 种形态；指定则只构建该形态（本仓 WEB 为独立镜像 aiot-edge-web）
...
#         docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web:arm64   (WEB 为本仓独立镜像)
#         docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web-standard:amd64
...
#         docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web:v1.0.0
#
# 镜像映射（远程 → 本地）:
...
#   形态相关镜像（WEB，全量形态均构建/推送；本仓 WEB 为独立镜像，与 easyaiot 完全区分）:
#     docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web:amd64     → edge-web-service:latest          (edge/full)
#     docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web-mini:amd64 → edge-web-service:latest-mini     (mini)
#     docker.cnb.cool/soaring-xiongkulu/easyaiot-edge/aiot-edge-web-standard:amd64 → edge-web-service:latest-standard (standard)
...
all_build_plan_images_ready_for_arch() {
    ...
    if runtime_build_includes_module WEB; then
        for profile in "${build_profiles[@]}"; do
            local_image_ready "edge-web-service" "$profile" "$target_arch" || return 1
        done
    fi
    return 0
}
...
build_single_module() {
    ...
    case "$remote_name" in
        ...
        aiot-edge-web) build_module_with_install_script "WEB" "edge-web-service" "$local_ref" "$target_arch" ;;
...
_build_push_track() {
    ...
    if [ "$lname" = "edge-web-service" ] && [ -n "$profile" ] && is_native_arch "$target_arch"; then
...
extract_web_dist_from_native_images() {
    ...
    for profile in "${profiles[@]}"; do
        ...
        img_ref=$(local_ref "edge-web-service" "$profile")
...
build_all_modules() {
    ...
    for target_arch in "${build_archs[@]}"; do
        ...
        if runtime_build_includes_module WEB; then
            for profile in "${build_profiles[@]}"; do
                ...
                _build_push_track "aiot-edge-web" "edge-web-service" "$profile" "$target_arch"
    ...
    rm -rf "${PROJECT_ROOT}/WEB/dist-prebuilt" "${PROJECT_ROOT}/WEB/dist-prebuilt-mini" \
           "${PROJECT_ROOT}/WEB/dist-prebuilt-standard" "${PROJECT_ROOT}/WEB/dist-prebuilt-full" \
           "${PROJECT_ROOT}/WEB/dist-prebuilt-edge" 2>/dev/null || true
