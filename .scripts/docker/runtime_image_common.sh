...
INDEPENDENT_MODULES=(
    ...
    "aiot-edge-web|edge-web-service|WEB"
...
PROFILE_DEPENDENT_REMOTES=(aiot-edge-web)
...
runtime_normalize_build_module() {
    ...
    case "$raw" in
        ...
        web|aiot-web|aiot-edge-web) echo "WEB" ;;
...
runtime_remote_ref() {
    ...
    # edge 前端为独立镜像：远端走独立命名空间 easyaiot-edge/aiot-edge-web，
    # 本地 edge-web-service —— 与 easyaiot 的 aiot-web* / web-service 完全区分
    local image_name="${name}"
    [ "$name" = "aiot-edge-web" ] && image_name="easyaiot-edge/${name}"
    # aiot-edge-web 本名已含 edge，edge 形态不再追加 -edge 后缀（其余形态照常）
    if [ -n "$profile" ] && [ "$profile" != "full" ] && ! { [ "$name" = "aiot-edge-web" ] && [ "$profile" = "edge" ]; }; then
...
runtime_images_collect_check_refs() {
    ...
    case "$profile" in
        edge)
            # 零 DEVICE：仅 VIDEO + WEB（edge 前端为独立镜像 latest-edge）
            _out+=("video-service:${tag}")
            _out+=("edge-web-service:${tag}-edge")
            return 0
            ;;
    esac
    _out=(
    ...
    case "$profile" in
        mini)     _out+=("edge-web-service:${tag}-mini") ;;
        standard) _out+=("edge-web-service:${tag}-standard") ;;
        *)        _out+=("edge-web-service:${tag}") ;;
    esac
