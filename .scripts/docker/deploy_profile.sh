...
# edge：品牌覆盖 JSON（write_edge_branding_json）已移除——
# WEB 为 edge 独立镜像，Logo/登录背景/系统文字直接构建进镜像，不再由 nginx 运行时替换

# WEB：按形态生成运行时 _app.config.js（挂载进容器覆盖构建期 VITE_GLOB_*，共用镜像免重建切换形态开关）
write_web_runtime_app_config() {
    local root="${1:-}"
    local tenant_flag="$2" captcha_flag="$3" vite_profile="$4" edge_flag="$5"
    local conf_dir="${root}/WEB/conf"
    local conf_file="${conf_dir}/_app.config.js"
    mkdir -p "$conf_dir"
    # 变量名与 build/getConfigFileName.ts 对齐：__PRODUCTION__<SHORT_NAME>__CONF__
    cat > "$conf_file" <<EOF
// 由 deploy_profile.sh 按部署形态生成（EASYAIOT_DEPLOY_PROFILE=${EASYAIOT_DEPLOY_PROFILE:-full}）；勿手工编辑
window.__PRODUCTION__IOT_ADMIN__CONF__ = {
  VITE_GLOB_DEPLOY_PROFILE: "${vite_profile}",
  VITE_GLOB_APP_TENANT_ENABLE: "${tenant_flag}",
  VITE_GLOB_APP_CAPTCHA_ENABLE: "${captcha_flag}",
  VITE_GLOB_EDGE_STANDALONE: "${edge_flag}"
};
Object.freeze(window.__PRODUCTION__IOT_ADMIN__CONF__);
EOF
}
...
sync_web_deploy_profile_env() {
    ...
    # 运行时配置无论 WEB/.env 是否存在都要生成（compose 挂载依赖该文件存在）
    write_web_runtime_app_config "$root" "$tenant_flag" "$captcha_flag" "$vite_profile" "$edge_flag"
    [ -f "$web_env" ] || return 0
...
# 若 WEB 镜像构建时的形态与当前不一致，提示需 rebuild
# （VITE_GLOB_* 开关已由运行时 _app.config.js 覆盖，仅在镜像不含该机制时才需重建）
warn_web_rebuild_if_profile_changed() {
    local root="${1:-$(_deploy_profile_repo_root)}"
    ...
    if [ -f "$stamp" ] && [ "$(tr -d '[:space:]' < "$stamp")" != "$current" ]; then
        echo "⚠️  WEB 部署形态已从 $(tr -d '[:space:]' < "$stamp") 变为 ${current}；形态开关已通过运行时 _app.config.js 生效，重启 web-service 即可。若镜像为旧版本（无运行时配置机制），请执行 WEB/install_linux.sh build 重新构建"
    fi
}
