...
has_gpu = $has_gpu
# 推荐的镜像源列表（主源 + 备用，Docker 会按顺序尝试）
# 若 docker.1ms.run 出现 EOF/超时，可临时移除镜像源使用 Docker Hub 或仅保留备用源
recommended_mirrors = [
...
ensure_postgresql_password() {
    ...
    if ! docker ps --filter "name=postgres-server" --format "{{.Names}}" | grep -q "postgres-server"; then
        ...
        print_info "常见原因：镜像拉取不完整（如遇 EOF/超时）。可先执行: cd $SCRIPT_DIR && $COMPOSE_CMD -f $COMPOSE_FILE pull && $COMPOSE_CMD -f $COMPOSE_FILE up -d"
        return 1
    fi
...
init_databases() {
    ...
    DB_SQL_MAP["iot-edge-ai20"]="${sql_dir}/iot-edge-ai10.sql"
    DB_SQL_MAP["iot-edge-video20"]="${sql_dir}/iot-edge-video10.sql"
...
check_and_pull_images() {
    ...
    # 如果有缺失的镜像，才执行拉取（带重试与超时，避免镜像源慢/挂起导致脚本卡住）
    if [ $missing_images -gt 0 ]; then
        print_info "发现 $missing_images 个缺失镜像，开始拉取..."
        print_info "已存在 $existing_images 个镜像，跳过拉取"
        print_info "若拉取长时间无进度，可先执行: cd $SCRIPT_DIR && ./pull_without_mirror.sh 改用 Docker Hub 直连后再重新运行本脚本"
        # 单次拉取超时时间（秒），避免镜像源挂起时脚本一直卡住
        local pull_timeout=900
        local pull_cmd="$COMPOSE_CMD -f $COMPOSE_FILE pull"
        if command -v timeout &>/dev/null; then
            pull_cmd="timeout $pull_timeout $pull_cmd"
        fi
        ...
        while [ $attempt -le $max_pull_attempts ]; do
            if [ $attempt -gt 1 ]; then
                print_info "第 $attempt 次拉取尝试（共 $max_pull_attempts 次）..."
            fi
            $pull_cmd 2>&1 | tee -a "$LOG_FILE"
            ...
            if [ "$pull_exit" -eq 124 ]; then
                print_warning "镜像拉取超时（${pull_timeout} 秒），可能为镜像源不稳定或网络慢"
                print_info "建议先执行: cd $SCRIPT_DIR && ./pull_without_mirror.sh 再重新运行本脚本 install"
            fi
            attempt=$((attempt + 1))
            if [ $attempt -le $max_pull_attempts ]; then
                print_warning "镜像拉取未完全成功，10 秒后重试..."
                sleep 10
            fi
        done
        if [ $pull_ok -eq 1 ]; then
            ...
            print_warning "镜像拉取可能不完整（如遇 EOF/超时多为镜像源不稳定）"
            print_info "建议：1) 稍后重新运行本脚本重试；2) 或先执行: cd $SCRIPT_DIR && ./pull_without_mirror.sh 再重新运行本脚本；3) 或临时移除 Docker 镜像源后执行: cd $SCRIPT_DIR && $COMPOSE_CMD -f $COMPOSE_FILE pull"
        fi
    else
...
check_and_clean_ports() {
    ...
    if [ $has_conflict -eq 1 ]; then
        ...
        if [ $has_host_process -eq 1 ]; then
            ...
            echo ""
            echo "请选择操作："
            echo "  1. 尝试停止宿主机上的 Redis 服务（如果存在）"
...
cleanup_stale_containers() {
    ...
    # 检查是否有停止的容器需要清理
    local stale_containers=$(docker ps -a --filter "status=exited" --format "{{.Names}}" 2>/dev/null | grep -E "(postgres-server|redis-server|srs-server)" || echo "")
...
install_middleware() {
    ...
    if [ ${#failed_containers[@]} -gt 0 ]; then
        ...
        if printf '%s\n' "${failed_containers[@]}" | grep -qx "PostgresSQL"; then
            print_info "若为镜像拉取不完整（如日志中出现 EOF/超时），可重试: cd $SCRIPT_DIR && $COMPOSE_CMD -f $COMPOSE_FILE pull && $COMPOSE_CMD -f $COMPOSE_FILE up -d"
        fi
        print_info "查看详细日志命令:"
...
delete_databases() {
    ...
    # 定义需要删除的数据库列表
    local databases=("iot-edge-ai20" "iot-edge-video20" "ruoyi-vue-pro20")
