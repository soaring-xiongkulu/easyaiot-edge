...
/** 当前部署形态（VITE_GLOB_DEPLOY_PROFILE，默认 full；生产环境可被运行时 _app.config.js 覆盖） */
export function getDeployProfile(): DeployProfile {
  return normalizeDeployProfile(getAppEnvConfig().VITE_GLOB_DEPLOY_PROFILE);
}
...
/** 登录页租户开关（生产环境可被部署时挂载的 _app.config.js 覆盖，无需重建镜像） */
export function isLoginTenantEnabled(): boolean {
  return String(getAppEnvConfig().VITE_GLOB_APP_TENANT_ENABLE ?? 'true').trim().toLowerCase() !== 'false';
}

export function isLoginCaptchaEnabled(): boolean {
  return String(getAppEnvConfig().VITE_GLOB_APP_CAPTCHA_ENABLE ?? 'true').trim().toLowerCase() !== 'false';
}

/** edge 单机合装（与 mini 共用前端裁剪，但零 DEVICE / 无集群 Tab） */
export function isEdgeStandaloneDeployProfile(): boolean {
  return String(getAppEnvConfig().VITE_GLOB_EDGE_STANDALONE ?? 'false').trim().toLowerCase() === 'true';
}
