...
export function getDefaultPlatformBranding(): PlatformBrandingConfig {
  ...
  // edge 独立镜像：品牌图（Logo/登录背景）即内置默认图，仅需覆盖系统文字
  if (isEdgeStandaloneDeployProfile()) {
    const edgeTitle = envTitle.includes('Edge') ? envTitle : 'EasyAIoT Edge 边缘智能平台'
    return {
      platformName: edgeTitle,
      platformLogo: defaultLogo,
      platformLogoFileId: null,
      dashboardTitle: 'EasyAIoT Edge 算法预警监控平台',
      loginName: edgeTitle,
