...
# ARM架构基础镜像
ARM_BASE_IMAGE="python:3.11-slim-bookworm"
...
configure_arm_dockerfile() {
    ...
    if [ -f Dockerfile.arm.raspi ]; then
        print_info "使用现有的 Dockerfile.arm"
        cp Dockerfile.arm.raspi Dockerfile
    else
...
clean_compose_cache() {
    ...
    # 查找 compose 文件
    if [ -f "${SCRIPT_DIR}/docker-compose-raspi.yaml" ]; then
        compose_file="${SCRIPT_DIR}/docker-compose-raspi.yaml"
...
create_env_file() {
    if [ ! -f .env.docker ]; then
        ...
        if [ -f env.example ]; then
            ...
            # 树莓派没有 NVIDIA GPU,FFMPEG_HWACCEL=none直接走libx264软编码，跳过nvenc检测
            sed -i 's/^USE_GPU=.*/USE_GPU=False/' VIDEO/.env.docker
            grep -q '^FFMPEG_HWACCEL=' VIDEO/.env.docker \
            && sed -i 's/^FFMPEG_HWACCEL=.*/FFMPEG_HWACCEL=none/' VIDEO/.env.docker \
            || echo 'FFMPEG_HWACCEL=none' >> VIDEO/.env.docker
