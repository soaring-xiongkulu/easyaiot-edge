...
def _alert_images_base_dir() -> Path:
    """VIDEO 项目根目录下的 alert_images（与算法服务写入路径一致）"""
    return (Path(__file__).resolve().parent.parent.parent / 'alert_images').resolve()


@alert_bp.route('/static/<path:filepath>')
def serve_alert_static(filepath: str):
    """对外提供告警截图静态访问，对应库中 /app/alert_images/... 的相对路径部分"""
    base = _alert_images_base_dir()
    try:
        ...
        if path.startswith('/api/v1/buckets/') and '/objects/download' in path:
            try:
                ...
                try:
                    ...
                    if e.code == 'NoSuchKey':
                        return api_response(400, f'对象不存在: {object_name}')
                    raise
            except Exception as e:
