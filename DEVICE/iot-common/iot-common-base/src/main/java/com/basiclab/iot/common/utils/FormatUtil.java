...
public class FormatUtil {
    ...
    public static String getProductIdentificationByFile(String appType, String appProductIdentification) {
        if ("1".equals(appType)) {
            return appProductIdentification;
        } else {
            try {
                String productIdentification = FileUtil.readString(appProductIdentification, "UTF-8");
                if (!StringUtils.isEmpty(productIdentification)) {
                    return productIdentification.trim();
                }
            } catch (Exception e) {
