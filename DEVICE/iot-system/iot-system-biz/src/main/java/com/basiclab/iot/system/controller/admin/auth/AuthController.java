...
public class AuthController {
    ...
    /**
     * 供前端或其它服务在发起业务请求前探测访问令牌是否仍在服务端（如 Redis）有效。
     * 需携带与业务接口相同的 Authorization；若已登出或过期，由安全框架返回 401。
     */
    @GetMapping("/validate-session")
    @Operation(summary = "校验当前登录会话是否仍有效")
    public CommonResult<Boolean> validateSession() {
