...
<p style="font-size: 16px; line-height: 1.8; color: #555; font-weight: 400; margin: 20px 0;">
  EasyAIoT Edge 是一套完全开源的异构边缘算力平台，在同一套操作习惯、数据链路和运维体系下，<strong>同时支持华为昇腾、NVIDIA Jetson、瑞芯微 RK3588、算能 BM1688 以及海光/Intel/AMD x86</strong>等异构边缘算力。平台覆盖摄像头接入、算法任务编排、推理、告警、录像的全链路，采用 MIT 协议，无厂商锁定风险。
</p>
...
| **省学习与对接成本** | 运维和集成只熟悉一套模块（WEB / VIDEO / RUNTIME）和一套端口拓扑；换机器主要是换推理镜像与参数，而不是从零再造系统。 |
...
**EasyAIoT Edge** 用固定的 **WEB / VIDEO / RUNTIME** 三模块和 **固定端口 + 环境变量 + host 网络**，把「日常用的平台」和「跟芯片绑定的推理部分」分开：**界面、告警、录像这一套不变**；换昇腾、Jetson、RK、BM1688 或 x86，主要是换推理镜像、编排参数和 RUNTIME（C++）的构建目标，对齐 CANN、CUDA/TensorRT、RK NPU、TPU 与 x86 CPU/GPU 等常见路线，从而**用一套开源平台**串起信创、工控、ARM 盒子和 x86 服务器。

## 🧩 支持的芯片与典型用法

| 芯片 / 平台 | 典型硬件 / 场景 | 在本平台里做什么 |
|----------|-----------------|------------------|
| **华为昇腾** | Atlas 边缘推理卡、昇腾 NPU 一体机等 | 信创与国产化推理栈；与 VIDEO / 告警 / 录像同一套业务闭环 |
...
我们坚持 **Python + C++** 双语言混编架构，发挥各自优势：

- **Python**：流媒体处理、AI 算法编排、模型服务
- **C++**：高性能推理热点路径（RUNTIME），低延迟、省内存；同一套调度逻辑下，可按不同芯片做推理加速

在边缘场景中，模块间通过**环境变量 + 固定端口 + host 网络**直连，不依赖中心化注册发现，降低运维成本；**换芯不换业务**——管理端与 VIDEO 协议层保持稳定，推理层按芯片切换。
...
| **内存与资源** | **4GB 级目标**（精简服务集 + 可调 Worker/推理参数） |
| **部署拓扑** | 单机或少量节点，中间件与业务直连，无多租户 |
| **服务发现** | 固定端口 + 环境变量，**不依赖**中心化注册中心 |
| **网络** | 视频服务默认 `network_mode: host`，便于与摄像头/局域网互通 |
| **数据库** | 默认库名 `iot-video20` |
| **算力** | **昇腾 / Jetson / RK3588 / BM1688 / x86** 等：RUNTIME（C++）按目标芯片编排；可参考 NVIDIA Container Toolkit 等官方容器方案 |
...
EasyAIoT Edge 由三个核心模块组成，可独立部署：

| 模块 | 描述 |
|------|------|
| **WEB** | Vue 3 + Vite 管理端：摄像头、算法任务、模型、告警、权限等 |
| **VIDEO** | 视频与算法任务 Python 服务：设备接入（ONVIF/GB28181）、SRS 推流转发、算法任务生命周期管理、告警与录像 |
| **RUNTIME** | C++ 高性能边缘推理运行时：拉流 → 解码 → AI 推理；通过 MQTT 推送告警、HTTP 心跳回传 VIDEO；支持 realtime / snap / patrol 任务类型 |
...
2. VIDEO 通过 SRS 转发原画流供实时预览；根据算法任务配置，生成任务配置并拉起 RUNTIME 二进制
3. RUNTIME 拉流、解码并执行 AI 推理，通过 MQTT 推送告警、HTTP 心跳回传 VIDEO
4. VIDEO 根据布防规则触发告警，事件落库，同时触发录像存储
5. WEB 前端统一展示设备状态、告警事件、录像回放
